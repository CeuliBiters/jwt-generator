<?php
require_once('../plugins/JWT-Framework/vendor/autoload.php');
...
class customECKey
{
    ...
    private static function createECKeyUsingOpenSSL(string $curve): array
    {
        ...
        $key = openssl_pkey_new([
            "config" => "C:/xampp1/php/extras/openssl/openssl.cnf",
        ...
        $result = openssl_pkey_export($key, $out, null, [
            "config" => "C:/xampp1/php/extras/openssl/openssl.cnf"
        ]);
        if (false === $result) {
            throw new RuntimeException('Unable to create the key');
...
class customJWKFactory extends JWKFactory
{
    /**
    ...
    public static function createECKey(string $curve, array $values = []): JWK
    {
        return customECKey::createECKey($curve, $values);
    }
}
